...
def fit_linear_uptake_monoexp_washout(
    A: np.ndarray,
    times: np.ndarray,
    valid: np.ndarray,
    ...
    """
    Fit linear uptake + mono-exponential washout model:
    
    For t <= t_max: A(t) = (A_max / t_max) * t
    For t > t_max: A(t) = A_max * exp(-lambda * (t - t_max))
    
    This model captures linear uptake phase followed by exponential clearance.
    Suitable for tumors with early imaging (< 24h).
...
def fit_linear_uptake_monoexp_washout_vectorized(
    A: np.ndarray,
    times: np.ndarray,
    valid: np.ndarray,
    lambda_phys: float | None,
    ...
    """
    Vectorized implementation of linear uptake + mono-exponential washout.
...
def tia_linear_uptake_monoexp_washout(params: np.ndarray, tpeak: np.ndarray) -> np.ndarray:
    """
    Compute TIA from linear uptake + mono-exponential washout parameters.
    
    TIA = 0.5 * A_max * t_max + A_max / lambda
    ...
    if np.any(ok):
        tia[ok] = 0.5 * A_max[ok] * t_max[ok] + A_max[ok] / lam[ok]
    
    return tia.astype(np.float32)


def fit_linear_uptake_monoexp_washout_matrix(
    A: np.ndarray,
    times: np.ndarray,
    valid: np.ndarray,
    lambda_phys: float | None,
