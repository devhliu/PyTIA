...
├── test_gamma_fix.py        # Gamma model fix tests
...
## Notes on legacy unit tests

Some `tests/unit/` files are still being migrated to match the current runtime API and
contract surface. The default release checklist therefore uses focused regression gates plus
contract checks, with an explicit `--full` option for full-suite enforcement.
